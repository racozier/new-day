  ...
  return (
    <div className="flex flex-col min-h-screen bg-cream-100">
      ...
      <div className="px-4 py-4 space-y-3">
        {areas.map(area => (
          <button key={area.key} onClick={() => navigate(area.to)}
            ...
            <div className="w-12 h-12 rounded-xl flex items-center justify-center text-xl flex-shrink-0"
              style={{ backgroundColor: AREA_COLORS[area.key] + '18' }}>
              {area.emoji}
            </div>
            <div className="flex-1">
              <p className="font-semibold text-navy-700">{area.label}</p>
              <p className="text-xs text-warm-400 mt-0.5">{area.desc}</p>
            </div>

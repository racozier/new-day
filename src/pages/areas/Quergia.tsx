  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-950">
      <PageHeader title="QUERGIA" subtitle={formatDate(DATE)} back
        right={<span className="text-sm font-bold px-3 py-1 rounded-lg bg-blue-500/20 text-blue-400">{score}%</span>}
      />
      <div className="flex-1 overflow-y-auto no-scrollbar px-4 py-4 space-y-4">
        {/* Metrics */}
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: 'Tasks', value: `${completed}/${entry.tasks.length}`, color: 'text-blue-400' },
            { label: 'Revenue', value: entry.revenue || '—', color: 'text-emerald-400', editable: true, key: 'revenue' as const },
            { label: 'Leads', value: entry.leads || '—', color: 'text-amber-400', editable: true, key: 'leads' as const },
          ].map(m => (
            <div key={m.label} className="bg-slate-900 rounded-xl p-3 text-center">
              ...
              {m.editable ? (
                <input
                  type="number"
                  defaultValue={typeof m.value === 'number' ? m.value : ''}
                  placeholder="0"
        ...
        {tab === 'today' && (
          <div className="space-y-2">
            {entry.tasks.length === 0 && !addingTask && (
              <p className="text-sm text-slate-600 text-center py-6">No tasks for today.<br /><span className="text-xs">Add from Master List or create new.</span></p>
            )}
            {entry.tasks.map(task => (
              <div key={task.id} className="flex items-center gap-3 bg-slate-900 rounded-xl px-3 py-2.5">
                <button onClick={() => toggleTask(task.id)}
                  className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${task.completed ? 'bg-emerald-500 border-emerald-500' : 'border-slate-600'}`}>
                  {task.completed && <Check size={12} strokeWidth={3} className="text-white" />}
                </button>
                <span className={`flex-1 text-sm ${task.completed ? 'line-through text-slate-600' : 'text-slate-200'}`}>{task.title}</span>
                <button onClick={() => saveEntry({ tasks: entry.tasks.filter(t => t.id !== task.id) })} className="p-1 text-slate-700 active:text-red-400"><Trash2 size={14} /></button>
              </div>
            ))}
            {addingTask ? (
              <div className="bg-slate-900 rounded-2xl p-4 space-y-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setAddingTask(false)} className="flex-1 py-2.5 rounded-xl bg-slate-800 text-slate-400 text-sm">Cancel</button>
        ...
        {tab === 'master' && (
          <div className="space-y-2">
            {masterTasks.map(mt => {
              ...
              return (
                <div key={mt.id} className="flex items-center gap-3 bg-slate-900 rounded-xl px-3 py-2.5">
                  <button onClick={() => togglePin(mt)} className={`p-1 ${mt.pinned ? 'text-amber-400' : 'text-slate-700 active:text-amber-400'}`}><Pin size={14} /></button>
                  <div className="flex-1">
                    <p className="text-sm text-slate-200">{mt.title}</p>
                    <p className="text-xs text-slate-600">{mt.category}</p>
                  </div>
                  <button
                    onClick={() => addFromMaster(mt)}
                    className={`text-xs px-2.5 py-1 rounded-lg font-medium ${inToday ? 'bg-emerald-500/20 text-emerald-400' : 'bg-brand-500/20 text-brand-400 active:bg-brand-500/30'}`}
                  >
                    {inToday ? '✓ Added' : '+ Today'}
                  </button>
                  <button onClick={async () => { if (mt.id) { await db.masterTasks.delete(mt.id); setMasterTasks(prev => prev.filter(t => t.id !== mt.id)) } }} className="p-1 text-slate-700 active:text-red-400"><Trash2 size={14} /></button>
                </div>
              )
            })}
            {addingMaster ? (
              <div className="bg-slate-900 rounded-2xl p-4 space-y-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setAddingMaster(false)} className="flex-1 py-2.5 rounded-xl bg-slate-800 text-slate-400 text-sm">Cancel</button>
        ...
        <div className="bg-slate-900 rounded-2xl p-4">
          <h3 className="font-semibold text-slate-200 mb-3">📝 Business Notes</h3>
          <textarea
            value={entry.notes}
            onChange={e => setEntry(prev => ({ ...prev, notes: e.target.value }))}
            onBlur={e => saveEntry({ notes: e.target.value })}
            placeholder="Notes, ideas, observations..."
            rows={4}
